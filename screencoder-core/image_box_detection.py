        ...
        metrics = await page.evaluate("""
            () => {
                ...
                for (const el of all_potential_placeholders) {
                    ...
                    if (el_rect.width < 40 || el_rect.height < 40) continue;
                    ...
                    for (const region_el of region_containers) {
                        ...
                        const overlap_w = Math.max(0, Math.min(el_rect.right, region_rect.right) - Math.max(el_rect.left, region_rect.left));
                        const overlap_h = Math.max(0, Math.min(el_rect.bottom, region_rect.bottom) - Math.max(el_rect.top, region_rect.top));
