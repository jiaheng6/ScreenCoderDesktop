from utils import encode_image, Doubao, Qwen, GPT, Gemini, OpenCodeGo
...
PROMPT_DICT = {
    "sidebar": f"""这是一张页面侧边栏区域截图。用户额外要求：{user_instruction["sidebar"]}
请写一段完整的 HTML 和 Tailwind CSS，尽量还原截图中的布局、图标、尺寸、间距、颜色和文字。
只返回一个根 <div> 以及它内部的代码，不要返回 Markdown 代码块，不要解释。""",
    "header": f"""这是一张页面顶部栏区域截图。用户额外要求：{user_instruction["header"]}
请写一段完整的 HTML 和 Tailwind CSS，尽量还原截图中的布局、按钮、输入框、图标、文字、尺寸、间距和颜色。
只返回一个根 <div> 以及它内部的代码，不要返回 Markdown 代码块，不要解释。""",
    "navigation": f"""这是一张页面导航区域截图。用户额外要求：{user_instruction["navigation"]}
请写一段完整的 HTML 和 Tailwind CSS，尽量还原截图中的导航标签、间距、文字大小、颜色和选中状态。
只返回一个根 <div> 以及它内部的代码，不要返回 Markdown 代码块，不要解释。""",
    "main content": f"""这是一张页面主内容区域截图。用户额外要求：{user_instruction["main content"]}
请写一段完整的 HTML 和 Tailwind CSS，尽量还原卡片网格、标题、作者、观看量、时间、间距和颜色。
截图中的每一个真实图片区域都必须使用相同尺寸的灰色 <div> 占位块，统一添加 data-screencoder-placeholder="image" 属性和 bg-gray-400 类。不要为这些区域输出 <img>、背景图、渐变或远程图片，也不要遗漏上述属性和类，后续脚本会据此替换为原始截图裁剪图。
只返回一个根 <div> 以及它内部的代码，不要返回 Markdown 代码块，不要解释。"""
}
...
if __name__ == "__main__":
    ...
    # 初始化 OpenCode Go 模型客户端。
    bot = OpenCodeGo(model="minimax-m3")
