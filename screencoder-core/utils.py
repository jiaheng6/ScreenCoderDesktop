...
class Doubao(Bot):
    def __init__(self, key_path, patience=3, model="doubao-1.5-thinking-vision-pro-250428") -> None:
        ...
        if Ark is None:
            raise ImportError("缺少 volcengine-python-sdk[ark]，无法使用 Doubao。")
...
class Qwen(Bot):
    def __init__(self, key_path, patience=3, model="qwen2.5-vl-32b-instruct") -> None:
        super().__init__(key_path, patience)
        if OpenAI is None:
            raise ImportError("缺少 openai，无法使用 Qwen 兼容接口。")
...
class GPT(Bot):
    def __init__(self, key_path, patience=3, model="gpt-4o") -> None:
        super().__init__(key_path, patience)
        if OpenAI is None:
            raise ImportError("缺少 openai，无法使用 GPT。")
...
class Gemini(Bot):
    def __init__(self, key_path, patience=3, model="gemini-1.5-flash-latest") -> None:
        super().__init__(key_path, patience)
        if genai is None:
            raise ImportError("缺少 google-generativeai，无法使用 Gemini。")
...
class OpenCodeGo(Bot):
    def __init__(
        ...
        key_path = key_path or os.environ.get("OPENCODE_API_KEY") or "opencode_api.txt"
        super().__init__(key_path, patience)
        if self.key == "opencode_api.txt":
            raise ValueError("请通过 OPENCODE_API_KEY 环境变量或 opencode_api.txt 提供 OpenCode Go API key。")
        self.model = model
        self.messages_url = f"{base_url.rstrip('/')}/messages"
