...
如果模型没有返回图片占位块，流水线会保留已经生成的初始 HTML，跳过 UIED 检测、映射和图片替换阶段，并正常生成最终产物；这类情况不会再导致整个任务失败。
...
4. 在“截图”页选择或拖入截图。
5. 在“运行”页选择目标框架和页面类型。
6. 运行页会自动检测环境；如果缺少依赖，点击“一键安装运行环境”，安装成功前“运行流水线”按钮保持禁用。
7. 点击“运行流水线”。
8. 在“日志”页查看实时阶段输出。
...
- 如果提示缺少 `cv2`、`PIL`、`bs4`、`playwright` 等模块，请等待“运行”页自动检测完成，然后点击“一键安装运行环境”。安装完成后流水线会优先使用应用托管虚拟环境。
